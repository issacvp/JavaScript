...
function canTwoMoviesFillFlight(movieLengths, flightLength) {
    ...
    for (let movieLength of movieLengths) {
        if (movieLength in movieMap) {
            movieMap[movieLength] += 1;
        } else {
            movieMap[movieLength] = 1;
        }
    }
    for (let movieLength in movieMap) {
        const diff = flightLength - movieLength;
        if (diff == movieLength) {                      // <== Watch out for the types , movieLength is a string (key of map) 
            if (movieMap[movieLength] === 2) {
                return true;
            }
        } else {
            if (diff in movieMap) {
